...
public interface UserAlgorithmMapper {
    ...
    @Select("select * from user_algorithm where userID = #{userID} group by algorithm order by count(algorithm) desc limit 1")
    ...
    @Select("select * from user_algorithm where userID = #{userID} and type = 'LEARN' group by algorithm order by count(algorithm) desc limit 1")
    UserAlgorithm findFavoriteLearningAlgorithmByUserID(int userID);

    @Select("select * from user_algorithm where userID = #{userID} and type = 'PRACTICE' group by algorithm order by count(algorithm) desc limit 1")
